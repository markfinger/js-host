...
Host.prototype.getListener = function getListener() {
  ...
  if (this.config.debug) {
    listener.get('*', this.getDebugGetHandler());
  }
...
Host.prototype.getDebugGetHandler = function getDebugGetHandler() {
  return function debugGetHandler(req, res) {
    var serviceNames = Object.keys(this.services);
    var output = '<html><body>';
    output += '<h1>Config</h1><p>' + JSON.stringify(this.config) + '</p>';
    output += '<h1>Services</h1>';
