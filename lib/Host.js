...
Host.prototype.getSerializableConfig = function getSerializableConfig() {
  var config = BaseServer.prototype.getSerializableConfig.call(this);

  // Convert `functions` to an array of names
  return _.mapValues(config, function(val, key) {
    return key === 'functions' ? Object.keys(val) : val;
  });
};

Host.prototype.addFunction = function addFunction(config) {
  var func = new Func(config);
