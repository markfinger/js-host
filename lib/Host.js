...
Host.prototype.initServices = function initServices() {
  ...
  // A transformation step which allows service configs to be written in
  // a variety of ways. Internally we use arrays of objects.
  if (!_.isArray(services) && _.isObject(services)) {
    services = this.config.services = _.map(services, function(val, key) {
...
Host.prototype.callService = function callService(name, data, key, cb) {
  var service = this.services[name];
  if (!service) {
    return cb('Cannot find a service named "' + name + '"');
  }

  // Allow `data` and `key` to be optional arguments
  if (_.isFunction(key)) {
    cb = key;
    key = undefined;
...
Host.prototype.getServiceCache = function getServiceCache() {
  return function serviceCache(req, res, next) {
    ...
    if (key) {
      var cachedOutput = req.service.getCachedOutput(key);
      if (cachedOutput) {
        this.logger.info('Service: ' + req.service.name + ' - cache hit for key: ' + key);
        return res.end(cachedOutput);
      }
      this.logger.info('Service: ' + req.service.name + ' - cache miss for key: ' + key);
      res.locals.key = key;
    }
    next();
  }.bind(this);
...
Host.prototype.getServiceResponse = function getServiceResponse() {
  return function serviceResponse(req, res) {
    ...
    return req.service.call(req.body, res.locals.key, done);
  }.bind(this);
};
