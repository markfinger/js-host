...
var Manager = function Manager(config) {
  ...
  if (!this.config.silent) {
    process.on('uncaughtException', function() {
      this.logger.info('Uncaught exception encountered, attempting to shut down all child processes');
      _.forEach(this.hosts, function(host) {
        try {
          host.process.kill();
        } catch(err) {
          this.logger.error('Failed to kill host at pid ' + host.process.pid);
        }
      }, this);
      this.logger.info('Completed attempt to shut down all child processes');
    }.bind(this));
  }
};
