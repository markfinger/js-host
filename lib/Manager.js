...
Manager.prototype.listen = function(cb) {
  ...
  // TODO: switch to JSON middleware, rather than www form
  this.app.use(this.getDataMiddleware());
...
Manager.prototype.serviceCacheMiddleware = function serviceCacheMiddleware(req, res, next) {
  ...
  if (cacheKey) {
    ...
    if (cachedOutput) {
      res.send(cachedOutput);
    }
  }
  next();
};

Manager.prototype.getDataMiddleware = function getDataMiddleware() {
...
Manager.prototype.router = function router(req, res) {
  //return res.status(404).send('Not found');

  var name = req.url.slice(1);

  return this.services[name].call(
    req.body,
    // TODO: cache key from header
